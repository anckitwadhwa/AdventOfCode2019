...
class NetworkNode:
    ...
    #TODO: should NetworkNode extend IntcodeExecutor
    # alternately change IOInterpretor abstraction in IntcodeExecutor to IOHandler. That could ge rid of the hack below where we are putting in -1s in the process_output function
    def __init__(self, nic_program, node_id, network_queue, debug_prints_level = 0):
        debug_prints_level -= 1
    
        self.input_queue = deque()
        self.output_queue = deque()
    
        self.input_queue.append(node_id)
        
        self.executor = IntcodeExecutor.IntcodeExecutor(nic_program, self.input_queue, self.output_queue, io_interpreter = None, debug_prints = False)
        self.network_queue = network_queue
        
        self.output_thread = threading.Thread(name="process_output for node:" + str(node_id), target=self.__process_output, args=([debug_prints_level]))
    ...
    def __del__(self):
        ...
        my_thread_join(self.executor.thread, currentframe().f_lineno, debug_prints_level)

    def __process_output(self, debug_prints_level = 0):
        debug_prints_level -= 1
        ...
        if(debug_prints_level > 90):
            print("Sleeping in __process_output")
            
        while(not(executor.is_halted())):
        
            # TODO: perhaps this should be moved to a separate process_input thread. If it does move, then we wont need to repeat it again a few lines below
            # make sure there are enough -1s in the input thread
            if(len(input_queue) < 1):
                input_queue.append(-1)
                
            while(not(executor.is_halted()) and (len(output_queue) < PACKET_LENGTH)):
                
                # TODO: perhaps this should be moved to a separate process_input thread
                # make sure there are enough -1s in the input thread
                if(len(input_queue) < 1):
                    input_queue.append(-1)
            ...
            destination_address = output_queue.popleft()
            x_value = output_queue.popleft()
            y_value = output_queue.popleft()
            
            network_queue.append((destination_address, x_value, y_value))
    
    def signal_termination(self, debug_prints_level = 0):
        debug_prints_level -= 1
        
        self.executor.signal_termination(False)
        
        
class NetworkProcessor:
    NAT_ADDRESS = 255
    RESTART_NETWORK_ACTIVITY_THRESHOLD_IN_SECS = 5
    ...
    def __init__(self, nic_program, num_nodes = 50, debug_prints_level = 0):
        ...
        for node_id in range(num_nodes):
            ...
            self.nodes[node_id] = NetworkNode(nic_program.copy(), node_id, self.network_queue, debug_prints_level)
    ...
    def __del__(self):
        ...
        for node in self.nodes:
            ...
            my_thread_join(node.output_thread, currentframe().f_lineno, debug_prints_level)
    ...
    def process_network_traffic(self, debug_prints_level = 0):
        ...
        while(not(self.is_halted())):
            ...
            if(destination_address < self.num_nodes):
                ...
                input_queue = nodes[destination_address].input_queue
                
                input_queue.append(x_value)
                input_queue.append(y_value)
            
            if(destination_address == self.NAT_ADDRESS):
                print(f'y_value for address {destination_address} is {y_value}')

                if(self.NAT_y == y_value):
                    print(f'first value delivered twice: {y_value}')
                    break
